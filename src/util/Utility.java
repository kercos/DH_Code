...
public class Utility {
	...
	public static void printChart(boolean[][] array) {
		StringBuilder result = new StringBuilder();
		for (int i = 0; i < array.length; i++) {
			for (int j = 0; j < array[i].length; j++) {
				if (j>0)
					result.append('\t');
				result.append(array[i][j] ? 'x' : "");
			}
			result.append('\n');
		}
		System.out.println(result);		
	...
	public static String joinStringListToString(List<String> array,
			String separator) {
		if (array.size() == 0)
			return "";
	...
	public static <S> HashMap<S, Double> convertHashMapIntArrayDouble(
			HashMap<S, double[]> map) {
		HashMap<S, Double> result = new HashMap<S, Double>();
		for (Entry<S, double[]> e : map.entrySet()) {
			result.put(e.getKey(), e.getValue()[0]);
		}
		return result;
	}
	...
	public static <T> String tableIntToString(AbstractMap<T, int[]> table) {
		StringBuilder sb = new StringBuilder();
	...
	public static boolean equalsIgnoreCase(List<String> l1,
			List<String> l2) {
		if (l1.size()!=l2.size())
			return false;
